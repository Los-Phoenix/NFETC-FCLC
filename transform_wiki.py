...
def transform(types, map_dict):
    res = [map_dict[t] for t in types.split() if t in map_dict]
    if len(res) == 0:
        print(types)
    return ' '.join(res)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(config.WIKIM_ALL):
        os.makedirs(os.path.dirname(config.WIKIM_ALL))
